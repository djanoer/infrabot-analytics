...
function _gatherMigrationDataSource(config) {
  ...
  if (allVms.length === 0) {
    throw new Error(`Data VM di '${config[K.SHEET_VM]}' tidak ditemukan atau kosong.`);
  }
...
function calculateVmHealthScore(vmDataRow, vmHeaders, config, vmHistoryLogs = [], activeTickets = []) {
  ...
  const criticality = String(vmDataRow[critIndex] || "").toUpperCase().trim();
  const isOff = String(vmDataRow[stateIndex] || "").toLowerCase().includes("off");
