...
function handleConfigManager(update, action, config, userData) {
  ...
  const chatId = userEvent.message ? userEvent.message.chat.id : userEvent.chat.id;
  const statusMessageId = userEvent.message
    ? userEvent.message.message_id
    : isInitialCall
    ? null
    : userEvent.message.id;
  ...
  try {
    if (action === "start" || action === "back_to_main") {
      ...
      const keyboard = {
        inline_keyboard: [
          ...
          [
            {
              text: "❌ Batal / Tutup",
              callback_data: CallbackHelper.build("config_machine", "cancel_view", {}, config),
            },
          ],
        ],
      };
      ...
      if (key === KONSTANTA.KUNCI_KONFIG.STRATEGI_PENEMPATAN_OPTIMAL) {
        pesan += `<b>Pilihan yang tersedia:</b>\n`;
        pesan += `• <code>BALANCE</code>\n<i>(Default) Bot menyeimbangkan beban cluster dan kapasitas datastore untuk menemukan lokasi terbaik secara keseluruhan.</i>\n\n`;
        pesan += `• <code>DENSITY_FIRST</code>\n<i>Bot akan memprioritaskan datastore yang paling sepi (jumlah VM paling sedikit) untuk menyebar risiko.</i>\n\n`;
        pesan += `• <code>FILL_UP</code>\n<i>Bot akan memprioritaskan datastore yang hampir penuh untuk memaksimalkan utilisasi dan mengosongkan datastore lain.</i>\n\n`;
      }
